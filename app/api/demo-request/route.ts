...
export async function POST(request: NextRequest) {
  try {
    ...
    const verificationUrl = `https://lynqly-app.vercel.app/api/verify-demo?token=${verificationToken}`
    ...
    if (!resendApiKey) {
      ...
      return NextResponse.json({ 
        success: true,
        warning: 'Request saved but emails not sent (API key missing)'
      })
    }
    ...
    console.log('User email response:', userEmailResponse.status, userEmailData)

    if (!userEmailResponse.ok) {
      console.error('❌ User email failed:', userEmailData)
    } else {
      console.log('✅ User email sent:', userEmailData.id)
    }
    ...
    console.log('Admin email response:', adminEmailResponse.status, adminEmailData)
    ...
    return NextResponse.json({ 
      ...
      emailsSent: {
        user: userEmailResponse.ok,
        admin: adminEmailResponse.ok
      }
    })
